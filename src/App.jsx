...
export default function TilingEstimator() {
  ...
  const calculateTotals = () => {
    let area = 0, adhesive = 0, grout = 0, cement = 0, antiCrack = 0, tank = 0, trim = 0;
    rooms.forEach(r => {
    ...
    return { totalArea: area.toFixed(2), adhesiveBags, groutBags, cementBoards, antiCrackMembrane: antiCrack.toFixed(2), tankingTubs, trimLengths, labourCost: labourCost.toFixed(2), materialsCost: matCost.toFixed(2), profitAmount: (matCost - baseMat).toFixed(2), totalCost: (matCost + labourCost).toFixed(2) };
  };
          ...
          <div className="bg-white rounded shadow p-4">
            ...
            <div className="mb-6">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {[['name', 'Name'], ['phone', 'Phone'], ['email', 'Email'], ['address', 'Address']].map(([k, l]) => (
                  <div key={k}><label className="block text-xs mb-1">{l}</label><input type="text" value={customer[k]} onChange={(e) => setCustomer({...customer, [k]: e.target.value})} className="w-full px-2 py-1.5 border rounded text-sm" /></div>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              <div className="flex justify-between mb-3">
                ...
                <button onClick={() => setRooms([...rooms, { id: rooms.length + 1, name: `Room ${rooms.length + 1}`, length: '', width: '', surfaceType: 'floor', trowelSize: '6', useCementBoard: false, useAntiCrack: false, useTanking: false, useGroutCalc: false, tileWidth: '', tileHeight: '', groutWidth: '3', tileThickness: '10', useTrim: false, trimLength: '', notes: '', notesPrice: '' }])} className="px-2 py-1 bg-blue-600 text-white rounded text-xs">Add</button>
              </div>
              {rooms.map(r => (
